...
def _swap : Ident := identOfString "swap"
def _t : Ident := identOfString "t"
def _t'1 : Ident := (Positive.ofNat 128)

def f_swap : Function := {
  fn_return := tvoid,
  fn_callconv := cc_default,
  fn_params := [(_a, (tptr tuint)), (_b, (tptr tuint))],
  fn_vars := [],
  fn_temps := [(_t, tuint), (_t'1, tuint)],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Sset _t (Expr.Ederef (Expr.Etempvar _a (tptr tuint)) tuint))
  (Stmt.Ssequence
    (Stmt.Ssequence
      (Stmt.Sset _t'1 (Expr.Ederef (Expr.Etempvar _b (tptr tuint)) tuint))
      (Stmt.Sassign (Expr.Ederef (Expr.Etempvar _a (tptr tuint)) tuint)
        (Expr.Etempvar _t'1 tuint)))
    (Stmt.Sassign (Expr.Ederef (Expr.Etempvar _b (tptr tuint)) tuint)
      (Expr.Etempvar _t tuint))))
}
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_swap, GlobDef.Gfun (FunDef.Internal f_swap))]
