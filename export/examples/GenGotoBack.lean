...
namespace Info
  ...
  def source_file : String := "gotoback.c"
...
def _again : Ident := identOfString "again"
def _level : Ident := identOfString "level"
def _lookup : Ident := identOfString "lookup"
def _main : Ident := identOfString "main"

def f_lookup : Function := {
  fn_return := tuint,
  fn_callconv := cc_default,
  fn_params := [(_level, tuint)],
  fn_vars := [],
  fn_temps := [],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Slabel _again
    (Stmt.Sifthenelse (Expr.Ebinop Binop.Oeq (Expr.Etempvar _level tuint)
                        (Expr.Econst_int (Integers.Int.repr 0) tint) tint)
      (Stmt.Sreturn (some (Expr.Econst_int (Integers.Int.repr 7) tint)))
      Stmt.Sskip))
  (Stmt.Ssequence
    (Stmt.Sset _level
      (Expr.Ebinop Binop.Osub (Expr.Etempvar _level tuint)
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_lookup, GlobDef.Gfun (FunDef.Internal f_lookup))]

def public_idents : List Ident :=
[_lookup, ___builtin_debug, ___builtin_fmin, ___builtin_fmax,
