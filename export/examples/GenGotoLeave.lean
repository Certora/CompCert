...
namespace Info
  ...
  def source_file : String := "gotoleave.c"
...
def _leave_loop : Ident := identOfString "leave_loop"
def _main : Ident := identOfString "main"
def _n : Ident := identOfString "n"

def f_leave_loop : Function := {
  fn_return := tuint,
  ...
  fn_body :=
(Stmt.Ssequence
  (Stmt.Sset _acc (Expr.Econst_int (Integers.Int.repr 0) tint))
  (Stmt.Ssequence
    (Stmt.Sloop
      (Stmt.Ssequence
        Stmt.Sskip
        (Stmt.Ssequence
          (Stmt.Sifthenelse (Expr.Ebinop Binop.Oeq (Expr.Etempvar _n tuint)
          ...
          (Stmt.Ssequence
            (Stmt.Sset _acc
              (Expr.Ebinop Binop.Oadd (Expr.Etempvar _acc tuint)
                (Expr.Etempvar _n tuint) tuint))
            (Stmt.Sset _n
              (Expr.Ebinop Binop.Osub (Expr.Etempvar _n tuint)
                (Expr.Econst_int (Integers.Int.repr 1) tint) tuint)))))
      Stmt.Sskip)
    (Stmt.Slabel _done (Stmt.Sreturn (some (Expr.Etempvar _acc tuint))))))
}
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_leave_loop, GlobDef.Gfun (FunDef.Internal f_leave_loop))]

def public_idents : List Ident :=
[_leave_loop, ___builtin_debug, ___builtin_fmin, ___builtin_fmax,
