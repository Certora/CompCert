...
namespace Info
  ...
  def source_file : String := "localvar.c"
...
def _b : Ident := identOfString "b"
def _main : Ident := identOfString "main"
def _s : Ident := identOfString "s"
def _sink : Ident := identOfString "sink"
def _st : Ident := identOfString "st"
def _use_local : Ident := identOfString "use_local"
def _x : Ident := identOfString "x"
def _t'1 : Ident := (Positive.ofNat 128)

def f_use_local : Function := {
  fn_return := tuint,
  fn_callconv := cc_default,
  fn_params := [(_x, tuint)],
  fn_vars := [(_s, (Ty.Tstruct _st noattr))],
  fn_temps := [(_t'1, tuint)],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Sassign (Expr.Efield (Expr.Evar _s (Ty.Tstruct _st noattr)) _a tuint)
    (Expr.Etempvar _x tuint))
  (Stmt.Ssequence
    (Stmt.Sassign
      (Expr.Efield (Expr.Evar _s (Ty.Tstruct _st noattr)) _b tuint)
      (Expr.Econst_int (Integers.Int.repr 7) tint))
    (Stmt.Ssequence
      (Stmt.Scall (some _t'1)
        (Expr.Evar _sink (Ty.Tfunction [(tptr (Ty.Tstruct _st noattr))] tuint
                           cc_default))
        [(Expr.Eaddrof (Expr.Evar _s (Ty.Tstruct _st noattr))
...
[(CompositeDef.Composite _st SU.Struct
   [(Member.Member_plain _a tuint), (Member.Member_plain _b tuint)]
   noattr)]
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_sink,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "sink"
                                   (mksignature [XType.Xptr] XType.Xint
                                     cc_default))
     [(tptr (Ty.Tstruct _st noattr))] tuint cc_default)),
 (_use_local, GlobDef.Gfun (FunDef.Internal f_use_local))]

def public_idents : List Ident :=
[_use_local, _sink, ___builtin_debug, ___builtin_fmin, ___builtin_fmax,
