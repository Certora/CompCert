...
namespace Info
  ...
  def source_file : String := "test/lean/structcopy.c"
...
def _dst : Ident := identOfString "dst"
...
def f_put : Function := {
  ...
  fn_params := [(_dst, (tptr (Ty.Tstruct _code noattr)))],
  fn_vars := [(_here, (Ty.Tstruct _code noattr))],
  fn_temps := [],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Sassign
    (Expr.Efield (Expr.Evar _here (Ty.Tstruct _code noattr)) _op tuchar)
    (Expr.Econst_int (Integers.Int.repr 0) tint))
  (Stmt.Ssequence
    (Stmt.Sassign
      (Expr.Efield (Expr.Evar _here (Ty.Tstruct _code noattr)) _bits tuchar)
      (Expr.Econst_int (Integers.Int.repr 5) tint))
    (Stmt.Ssequence
      (Stmt.Sassign
        (Expr.Efield (Expr.Evar _here (Ty.Tstruct _code noattr)) _val
          tushort) (Expr.Econst_int (Integers.Int.repr 42) tint))
      (Stmt.Ssequence
        (Stmt.Sassign
          (Expr.Ederef (Expr.Etempvar _dst (tptr (Ty.Tstruct _code noattr)))
            (Ty.Tstruct _code noattr))
          (Expr.Evar _here (Ty.Tstruct _code noattr)))
        (Stmt.Sreturn none)))))
}

def composites : List CompositeDef :=
[(CompositeDef.Composite _code SU.Struct
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_put, GlobDef.Gfun (FunDef.Internal f_put))]

def public_idents : List Ident :=
[_put, ___builtin_debug, ___builtin_fmin, ___builtin_fmax, ___builtin_fnmsub,
