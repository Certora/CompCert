...
namespace Info
  ...
  def source_file : String := "test/lean/u16loop.c"
...
def _zero16 : Ident := identOfString "zero16"

def f_zero16 : Function := {
  fn_return := tvoid,
  fn_callconv := cc_default,
  fn_params := [],
  fn_vars := [(_count, (tarray tushort 16))],
  fn_temps := [(_len, tint)],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Ssequence
    ...
    (Stmt.Sloop
      (Stmt.Ssequence
        ...
        (Stmt.Sassign
          (Expr.Ederef
            (Expr.Ebinop Binop.Oadd (Expr.Evar _count (tarray tushort 16))
              (Expr.Etempvar _len tint) (tptr tushort)) tushort)
          (Expr.Econst_int (Integers.Int.repr 0) tint)))
      (Stmt.Sset _len
        (Expr.Ebinop Binop.Oadd (Expr.Etempvar _len tint)
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_zero16, GlobDef.Gfun (FunDef.Internal f_zero16))]

def public_idents : List Ident :=
[_zero16, ___builtin_debug, ___builtin_fmin, ___builtin_fmax,
