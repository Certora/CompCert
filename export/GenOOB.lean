...
namespace Info
  ...
  def source_file : String := "test/lean/oob.c"
...
def _a : Ident := identOfString "a"
def _arr : Ident := identOfString "arr"
def _i : Ident := identOfString "i"
def _main : Ident := identOfString "main"
def _read_oob : Ident := identOfString "read_oob"
def _t'1 : Ident := (Positive.ofNat 128)

def f_read_oob : Function := {
  fn_return := tushort,
  fn_callconv := cc_default,
  fn_params := [(_a, (tptr tushort))],
  fn_vars := [],
...
(Stmt.Ssequence
  (Stmt.Sset _t'1
    (Expr.Ederef
      (Expr.Ebinop Binop.Oadd (Expr.Etempvar _a (tptr tushort))
        (Expr.Econst_int (Integers.Int.repr 9) tint) (tptr tushort)) tushort))
...
def f_main : Function := {
  ...
  fn_callconv := cc_default,
  fn_params := [],
  fn_vars := [(_arr, (tarray tushort 9))],
  fn_temps := [(_i, tint), (_t'1, tushort)],
  fn_body :=
(Stmt.Ssequence
  (Stmt.Ssequence
    (Stmt.Ssequence
      ...
      (Stmt.Sloop
        (Stmt.Ssequence
          ...
          (Stmt.Sassign
            (Expr.Ederef
              (Expr.Ebinop Binop.Oadd (Expr.Evar _arr (tarray tushort 9))
                (Expr.Etempvar _i tint) (tptr tushort)) tushort)
            (Expr.Ecast (Expr.Etempvar _i tint) tushort)))
        (Stmt.Sset _i
          (Expr.Ebinop Binop.Oadd (Expr.Etempvar _i tint)
            (Expr.Econst_int (Integers.Int.repr 1) tint) tint))))
    (Stmt.Ssequence
      (Stmt.Scall (some _t'1)
        (Expr.Evar _read_oob (Ty.Tfunction [(tptr tushort)] tushort
                               cc_default))
        [(Expr.Evar _arr (tarray tushort 9))])
      (Stmt.Sreturn (some (Expr.Ecast (Expr.Etempvar _t'1 tushort) tint)))))
...
[(___compcert_va_int32,
 ...
 (___builtin_debug,
   GlobDef.Gfun (FunDef.External (ExtFun.EF_external "__builtin_debug"
     ...
     [tint] tvoid
     { cc_vararg := (some 1), cc_unproto := false, cc_structret := false })),
 (_read_oob, GlobDef.Gfun (FunDef.Internal f_read_oob)),
 (_main, GlobDef.Gfun (FunDef.Internal f_main))]

def public_idents : List Ident :=
[_main, _read_oob, ___builtin_debug, ___builtin_fmin, ___builtin_fmax,
